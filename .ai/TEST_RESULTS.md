...
**Initial Setup tutorial and guide verification — 2026-08-07:** Registered `UI-ADM-013` in `TutorialRegistry` with six localized tour steps covering Hero, readiness summary, next-step CTA, checklist, financial approval boundary, and safety callout. `/help/screens/UI-ADM-013` rendered the full guide with five scoped actions, four field explanations, warnings, recovery guidance, FAQ, and related workflows. `/help/flows/FLW-ADM-06` rendered the ordered five-step owner workflow. Arabic Page Guide and interactive tour rendered with `lang=ar`, `dir=rtl`, Arabic headings/body/buttons, and `1 / 6` progress. Browser console returned 0 errors. The Demo Cashier boundary remains denied; no permissions are granted by the guide itself.

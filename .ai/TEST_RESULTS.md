...
**Implementation status:** TSK-015 local/dev verification completed; production/UAT remains open.
**Current diagnostics:** Locale parity 1035/1035, PHPStan 0 errors, Pint / PHP lint pass, Blade cache pass, route discovery pass, and git diff check pass on 2026-08-06. TSK-015 template workbook validated: 5 sheets, 14 canonical headers, 3 data validations, 0 formulas, 0 macros, no production data.
**Automated tests:** Not created or run per owner directive (no PHPUnit/Pest or automated browser tests claimed).
**TSK-015 local/dev evidence — 2026-08-06:** `migrate:fresh --force` on local SQLite and `DemoSeeder` completed successfully. Browser Demo Admin created a draft with `3 × 12.50`, `10%` discount, `14%` tax, and total `38.4750`; separate Demo Reviewer submitted approval and posting. Database evidence: `PINV-000001` approved, stock movement `3.000000`, WAC `11.2500`, on-hand `3.000000`, total value `33.7500`, one approval audit event. Reject and cancel transitions succeeded with required reasons; reverse returned on-hand/value to zero and created the inverse movement. Invalid CSV staged as `1 total / 0 valid / 1 invalid` with no new invoice or movement. Print route rendered bilingual invoice output; guest import route returned `302 → /login`. No automated PHPUnit/Pest or automated browser tests claimed.

**TSK-015 local default boundary:** approval posts automatic receipt/WAC, over-receipt is blocked, reversal requires sufficient on-hand, import creates Draft only, `PINV-` is a local numbering default, and sale prices are not mutated. These are engineering defaults, not production Owner Approvals; unresolved `.ai/TSK-015_OWNER_INPUTS.md`, UAT, and release gates remain open.

**Manual browser verification:** TSK-014 authenticated manual browser verification completed for approved local/demo scope; true 390x844 mobile evidence remains pending. TSK-015 settings route guest boundary verified (`302 → /login`); authenticated settings screen walkthrough remains pending because no safe authenticated browser credentials were available.

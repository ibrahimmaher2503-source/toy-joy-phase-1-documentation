...
The Initial Setup Dashboard / First Launch Configuration slice is implemented for Local/Dev. `/dashboard` derives readiness and shows the setup panel; `/initial-setup` is permission-gated by `company_settings.edit` and links to existing authoritative data-entry screens. After explicit owner authorization, Local Demo Administrator sees `3/5` required steps complete (`60%`) with `TOY & JOY - Local Demo / EGP / ج.م`; Local Demo Cashier received HTTP 403/Access Denied. Supplier-return reasons remain empty and no approved/effective financial versions or production sign-off were created.

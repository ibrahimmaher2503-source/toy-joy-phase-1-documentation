...
TSK-016 is **In Progress under DEC-052**. Implemented local/dev foundation: `supplier_return_reasons`, `purchase_returns`, and `purchase_return_lines` migrations/models; separate `purchase_returns.*` permissions; guarded draft creation requiring an approved source invoice line, active reason, eligible quantity, current on-hand, and original source `unit_cost`; submit/approve actions; `purchase_return` stock movement at negative original cost; WAC/balance update; idempotent approval; and `/purchasing/returns` list/editor/empty-state UI. Production master data, reason catalog rows, numeric limits, UAT, and Phase gates remain open.

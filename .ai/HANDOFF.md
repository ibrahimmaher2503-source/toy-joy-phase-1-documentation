# Active Handoff — TSK-038 Dashboards and Reconciled Report Catalog — 2026-08-08

TSK-038 is now active. First inspect `docs/34-reporting-dashboard-alerts-specification.md`, `docs/36`, `docs/37`, `docs/38`, `docs/39`, existing dashboard/report/source models, formula and lineage contracts, scope permissions, pagination, export, alert state, and current UI. The authorized scope is Local/Dev discovery/readiness only: configurable `PENDING/TBD` KPI/report groups, source lineage, formula, scope, date/branch/store filters, pagination, export, and alert values, guarded empty/readiness UI, and bilingual browser evidence. No production KPI truth, cross-scope data, export artifact, alert mutation, or unsupported financial value may be enabled.

TSK-037 is complete for its bounded Local/Dev quotation/proposal readiness slice; quote creation, approval, price/number allocation, print/share, conversion, sale, party invoice, inventory, wallet, payment, and financial mutations remain blocked. The quotation route uses existing `dashboard_reports.view` only as a temporary read-only guard because no canonical quotation permission exists.

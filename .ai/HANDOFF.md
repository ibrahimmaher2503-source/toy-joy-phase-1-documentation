...
TSK-015 local/dev implementation and verification are complete for the invoice/receipt foundation: versioned settings schema/read-only screen, manual draft CRUD, BCMath tax/discount calculation, private staged Excel/CSV import, row validation/formula-like-cell rejection, duplicate/idempotency checks, submit/reject/cancel/approve/reverse lifecycle, strict PO matching/no over-receipt, automatic receipt on approval under the local default, stock movement/balance/WAC update with reversal, audit, print, CSV export, and permission boundaries. Fresh local SQLite rebuild plus `DemoSeeder` passed. Browser/database evidence includes correct percentage totals (`38.4750`), separate reviewer approval, stock/WAC reconciliation, reject/cancel reasons, reversal, invalid import isolation, print rendering, and guest redirect.

The adopted local defaults are explicit engineering defaults only: approval receives stock automatically, over-receipt is blocked, `PINV-` numbering is allocated only on approval, reversal requires sufficient on-hand, import creates Draft invoices only, and sale-price mutation is not performed. They do not answer production Owner Inputs or close UAT/release gates.

Current verification: targeted PHPStan 0 errors, Pint/PHP lint pass, Blade cache pass, route discovery pass, fresh migration/seed pass, invalid-import smoke pass with `1 invalid / 0 new invoices / 0 new movements`, lifecycle/WAC evidence pass, guest browser redirect pass for `/purchasing/invoices/import`, and `git diff --check` pass. No PHPUnit/Pest or automated browser tests claimed. The remaining gap observed during this run is that the import Review Livewire update produced an empty browser DOM in the available automation session; the backend staging result and import landing page are verified, but Review UI should receive a separate fix/verification before production/UAT claims.

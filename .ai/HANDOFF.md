...
TSK-015 Slice B foundation is now implemented locally: `financial_setting_versions` is an append/version schema with effective and lock timestamps, creator/audit linkage fields, and unique `(key, version)` protection. The read-only screen is available at `/purchasing/invoices/settings`, gated by `company_settings.view`; it shows the empty state and keeps settings writes disabled. The targeted migration passed and created zero rows. Full `php artisan migrate` remains blocked by pre-existing SQLite drift (`categories` already exists); no historical migration was changed.

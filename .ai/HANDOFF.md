# Active Handoff — TSK-041 Approved Master Data Import and Reconciliation — 2026-08-08

TSK-041 is now active. First inspect `docs/23-master-data-and-opening-stock.md`, `docs/25-data-import-and-cutover.md`, `docs/36`, `docs/37`, `docs/38`, `docs/39`, plus canonical template/import, audit, permissions, backup, and opening-stock documents discovered by repository mapping. The authorized scope is Local/Dev discovery/readiness only: configurable `PENDING/TBD` approved data/template, validation, dry-run, reconciliation, opening-stock, cutover, maker/checker, backup, and error values; guarded empty/readiness UI and bilingual browser evidence. No upload, parsing, batch persistence, destructive update, stock posting, production cutover, UAT sign-off, or production claim may be enabled.

TSK-040 is complete for its bounded Local/Dev export-center and audit-view readiness slice; PDF/Excel/CSV generation, protected downloads, artifact storage, redaction execution, audit export, UAT, and release approval remain blocked.

# Active Handoff — TSK-042 Production Readiness, Devices, Backup, and Training — 2026-08-08

TSK-042 is now active. Read `docs/30-platform-operations-specification.md`, `docs/34-reporting-dashboard-alerts-specification.md`, `docs/38-print-export-specification.md`, `docs/39-uat-and-release-gates.md`, `docs/53-deployment-backup-and-rollback-runbook.md`, and `docs/54-production-data-migration-and-reconciliation.md`, then inspect actual health/readiness routes, config/secrets handling, queues/scheduler/storage, printer/scanner/device records, backup/restore commands, support ownership, training, and runbooks. Authorized scope is Local/Dev discovery/readiness only. No production deployment, secret creation, device enrollment, backup deletion, restore claim, UAT sign-off, release approval, or production claim.

TSK-041 completed its bounded Local/Dev master-data import/cutover readiness slice. `/master-data-migration-readiness` is guarded by `company_settings.view`, bilingual, pending, no-upload/no-persistence/no-cutover, and browser-verified with UI-SYS-008 and 4/4 selectors. Production source files, import batches, opening-stock posting, and cutover remain blocked.

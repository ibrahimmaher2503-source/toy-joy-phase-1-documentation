...
TSK-015 local/dev implementation and verification are **completed for the local/demo boundary**: versioned settings foundation, manual draft CRUD, BCMath tax/discount calculation, private staged Excel/CSV import with row validation and formula-like-cell rejection, duplicate/idempotency checks, submit/reject/cancel/approve/reverse lifecycle, strict PO matching/no over-receipt, automatic receipt on approval under the local default, stock movement/balance/WAC update with reversal, audit events, print, CSV export, and permission boundaries. Fresh SQLite rebuild and full DemoSeeder passed; browser/database walkthrough evidence is recorded in `.ai/TEST_RESULTS.md`. Production/UAT acceptance, unresolved Owner Inputs, commercial settings, opening stock, and release readiness remain open. Diagnostics: locale parity 1035/1035, targeted PHPStan 0 errors, Pint/PHP lint pass, Blade cache pass, route discovery pass, invalid-import smoke pass, lifecycle/WAC evidence pass, guest redirect pass, and git diff check pass. No PHPUnit/Pest or automated browser tests claimed.

Local/dev defaults recorded for this engineering slice: invoice approval receives stock automatically; over-receipt is blocked; `PINV-` numbering is allocated only at approval; reversal requires sufficient on-hand; import creates Draft invoices only; sale-price mutation is not performed. These defaults are not production approvals.

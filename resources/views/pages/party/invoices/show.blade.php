    ...
    <div class="mx-auto w-full max-w-7xl space-y-6 p-4 sm:p-6">
        <x-page-header :title="$invoice->invoice_number" :description="__('Party-only working invoice. It freezes at operation and becomes immutable at final close.')">
            <x-slot:actions>
                ...
                <flux:button href="{{ route('parties.invoices.print', $invoice->id) }}" target="_blank" variant="subtle" icon="printer">{{ __('Print') }}</flux:button>
            </x-slot:actions>
        </x-page-header>
                        ...
                        <div class="space-y-3">
                            @foreach($invoice->lines as $i => $line)
                                <div class="grid gap-3 rounded-xl border border-border p-3 sm:grid-cols-12">
                                    ...
                                    <div class="sm:col-span-2"><label class="block text-xs font-semibold">{{ __('Consumable product ID') }}</label><input type="number" name="lines[{{ $i }}][product_id]" value="{{ $line->product_id }}" class="mt-1 w-full rounded-lg border-zinc-300 text-sm dark:border-zinc-700 dark:bg-zinc-900"></div>

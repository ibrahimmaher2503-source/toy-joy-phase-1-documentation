                ...
                <div class="mt-4 space-y-3">
                    @foreach(range(0, 2) as $i)
                        <div class="grid gap-3 rounded-xl border border-cyan-100 bg-white p-3 dark:border-cyan-900 dark:bg-zinc-900 sm:grid-cols-12">
                            ...
                            <div class="sm:col-span-4"><flux:input name="lines[{{ $i }}][description]" :label="__('Description')" :value="old('lines.'.$i.'.description')" required /></div>
                            <div class="sm:col-span-2"><flux:input name="lines[{{ $i }}][quantity]" type="number" step="0.000001" min="0.000001" :label="__('Quantity')" :value="old('lines.'.$i.'.quantity', $i === 0 ? '1' : '')" required /></div>
                            <div class="sm:col-span-2"><flux:input name="lines[{{ $i }}][unit_price]" type="number" step="0.0001" min="0" :label="__('Unit price')" :value="old('lines.'.$i.'.unit_price', $i === 0 ? '0' : '')" required /></div>
                            <div class="sm:col-span-2"><flux:input name="lines[{{ $i }}][product_id]" type="number" :label="__('Product ID for consumable')" :value="old('lines.'.$i.'.product_id')" /></div>

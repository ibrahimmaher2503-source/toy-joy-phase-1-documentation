...
use App\Actions\Platform\SaveBranchAction;
...
<section class="w-full space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex items-center gap-3">
            <flux:button icon="plus" variant="primary" size="sm" wire:click="openCreateBranchModal">
                {{ __('Add Branch') }}
            </flux:button>
        </div>
    </div>
    ...
    <!-- Data Table -->
    <?php
    $query = Branch::with(['stores', 'activeSellingStoreMapping.store']);
    ...
    <flux:modal wire:model="showMappingModal" class="md:w-140 space-y-6">
        ...
        <?php
        $availableSellingStores = Store::where('type', 'selling')
            ->where('status', 'active')
            ->orderBy('code')
            ->get();
        ?>

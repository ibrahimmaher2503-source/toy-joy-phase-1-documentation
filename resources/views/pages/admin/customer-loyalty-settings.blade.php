    ...
    <div class="mx-auto w-full max-w-6xl space-y-6 p-4 sm:p-6">
        ...
        <div class="grid gap-4 lg:grid-cols-2">
            @foreach ($settings as $key => $setting)
                ...
                <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" data-setting-key="{{ $key }}" @if ($loop->first) data-guide="customer-settings-first-card" @endif>
                    <div class="flex items-start justify-between gap-3">
                        <div>
                            <h2 class="font-semibold text-slate-950" data-guide="customer-settings-first-card-heading">{{ __($setting['title']) }}</h2>

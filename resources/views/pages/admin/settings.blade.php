...
use App\Actions\Platform\SaveLocalSettingsAction;
        ...
        <div id="panel-payments" role="tabpanel" aria-labelledby="tab-payments" class="space-y-6">
            ...
            <flux:card class="space-y-4">
                ...
                <flux:table aria-label="{{ __('Configured Payment Methods') }}">
                    ...
                    <flux:table.rows>
                        @forelse (\App\Models\PaymentMethod::all() as $method)
        ...
        <div id="panel-tax" role="tabpanel" aria-labelledby="tab-tax" class="space-y-6">
            ...
            <flux:card class="space-y-4">
                ...
                <flux:table aria-label="{{ __('Configured Tax Settings') }}">
                    ...
                    <flux:table.rows>
                        @forelse (\App\Models\TaxSetting::all() as $tax)
        ...
        <div id="panel-sequences" role="tabpanel" aria-labelledby="tab-sequences" class="space-y-6">
            ...
            <flux:card class="space-y-4">
                ...
                <flux:table aria-label="{{ __('Configured Document Sequences') }}">
                    ...
                    <flux:table.rows>
                        @forelse (\App\Models\DocumentSequence::all() as $seq)
        ...
        <div id="panel-printers" role="tabpanel" aria-labelledby="tab-printers" class="space-y-6">
            ...
            <flux:card class="space-y-4">
                ...
                <flux:table aria-label="{{ __('Configured Printer Profiles') }}">
                    ...
                    <flux:table.rows>
                        @forelse (\App\Models\PrinterConfiguration::all() as $printer)
        ...
        <div id="panel-audit" role="tabpanel" aria-labelledby="tab-audit" class="space-y-6">
            <flux:card class="space-y-4">
                <div class="flex items-center justify-between">
                    <div>
                        <flux:heading size="lg">{{ __('Local Settings Audit Log (DEC-033)') }}</flux:heading>
                        <flux:subheading>{{ __('Append-only event record generated for successful settings modifications.') }}</flux:subheading>
                    </div>
                ...
                <flux:table aria-label="{{ __('Local Settings Audit Log') }}">
                    <flux:table.columns>
                        <flux:table.column>{{ __('Timestamp') }}</flux:table.column>
                        <flux:table.column>{{ __('Correlation ID') }}</flux:table.column>
                        <flux:table.column>{{ __('User') }}</flux:table.column>
                        <flux:table.column>{{ __('Action') }}</flux:table.column>
                        <flux:table.column>{{ __('Setting Type') }}</flux:table.column>
                        <flux:table.column>{{ __('Changes Summary') }}</flux:table.column>
                    </flux:table.columns>

                    <flux:table.rows>
                        @forelse (\App\Models\SettingsAuditLog::latest('id')->take(20)->get() as $log)
                            <flux:table.row key="log-{{ $log->id }}">
                                <flux:table.cell class="font-mono text-xs">{{ $log->created_at->format('Y-m-d H:i:s') }}</flux:table.cell>
                                <flux:table.cell class="font-mono text-xs text-blue-600 dark:text-blue-400">{{ $log->correlation_id }}</flux:table.cell>
                                <flux:table.cell class="font-medium text-xs">{{ $log->user_name ?? 'System' }}</flux:table.cell>
                                <flux:table.cell><flux:badge size="sm" color="teal">{{ $log->action }}</flux:badge></flux:table.cell>
                                <flux:table.cell class="font-mono text-xs">{{ $log->setting_type }}</flux:table.cell>
                                <flux:table.cell class="font-mono text-xs max-w-xs truncate" title="{{ json_encode($log->changes) }}">

<x-layouts::app :title="__('Sales')">
    <div class="mx-auto w-full max-w-7xl space-y-4 p-4">
        <div class="flex flex-wrap items-start justify-between gap-3"><div><flux:heading size="xl">{{ __('Sales') }}</flux:heading><flux:text class="mt-1">{{ __('Approved Local/Dev retail sales.') }}</flux:text></div><flux:button href="{{ route('pos') }}" variant="primary">{{ __('Open POS') }}</flux:button></div>

...
<x-layouts.app :title="$t('مركز تحكم المخزون', 'Inventory Control Center')">
    <div class="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8" data-inventory-demo>
        <div class="flex flex-col gap-4 rounded-3xl border border-amber-200 bg-gradient-to-br from-amber-50 via-white to-sky-50 p-6 shadow-sm dark:border-amber-900/50 dark:from-amber-950/30 dark:via-zinc-900 dark:to-sky-950/30 sm:flex-row sm:items-end sm:justify-between">
            <div>
        ...
        <section class="rounded-3xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900" aria-labelledby="balances-heading">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-start text-sm" data-stock-balance-table>
                    <thead class="bg-zinc-50 text-xs uppercase text-zinc-500 dark:bg-zinc-950/50"><tr><th class="px-5 py-3">{{ $t('المنتج', 'Product') }}</th><th class="px-5 py-3">{{ $t('المتجر', 'Store') }}</th><th class="px-5 py-3">On-hand</th><th class="px-5 py-3">Reserved</th><th class="px-5 py-3">Available</th><th class="px-5 py-3">In transit</th><th class="px-5 py-3">WAC</th><th class="px-5 py-3">{{ $t('إعادة الطلب', 'Reorder') }}</th></tr></thead>
                    <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                        @forelse ($balances as $balance)
                            ...
                            <tr data-stock-balance-row><td class="px-5 py-4 font-semibold text-zinc-900 dark:text-white">{{ $balance->product?->item_code }}<div class="text-xs font-normal text-zinc-500">{{ $isArabic ? $balance->product?->name_ar : $balance->product?->name_en }}</div></td><td class="px-5 py-4">{{ $balance->store?->code }}</td><td class="px-5 py-4 font-mono">{{ number_format((float) $balance->on_hand, 3) }}</td><td class="px-5 py-4 font-mono">{{ number_format((float) $balance->reserved, 3) }}</td><td class="px-5 py-4 font-mono font-bold {{ $available <= $reorder ? 'text-rose-700' : 'text-emerald-700' }}">{{ number_format($available, 3) }}</td><td class="px-5 py-4 font-mono">{{ number_format((float) $balance->in_transit, 3) }}</td><td class="px-5 py-4 font-mono">{{ number_format((float) $balance->average_cost, 4) }}</td><td class="px-5 py-4 font-mono">{{ number_format($reorder, 3) }}</td></tr>
                        @empty
                            <tr><td colspan="8" class="px-5 py-8 text-center text-zinc-500">{{ $t('لا توجد أرصدة.', 'No balances.') }}</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        <section class="rounded-3xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900" aria-labelledby="transfers-heading">
            ...
            <div class="space-y-4 p-5">
                @forelse ($transfers as $transfer)
                    <div class="rounded-2xl border border-zinc-200 p-4 dark:border-zinc-700" data-transfer-row>
                        ...
                        @if(in_array($transfer->status, ['in_transit', 'difference_review'], true))<form class="mt-4 grid gap-3 rounded-xl bg-zinc-50 p-3 dark:bg-zinc-950/60 sm:grid-cols-4" method="POST" action="{{ route('inventory.transfers.receive', $transfer) }}">@csrf<div><label class="text-xs font-semibold text-zinc-600 dark:text-zinc-300" for="received-{{ $transfer->id }}">{{ $t('الكمية المستلمة', 'Received quantity') }}</label><input id="received-{{ $transfer->id }}" name="received_quantity" value="{{ $transfer->lines->first()?->quantity_dispatched }}" inputmode="decimal" class="mt-1 w-full rounded-lg border-zinc-300 text-sm dark:border-zinc-700 dark:bg-zinc-900" required></div><div><label class="text-xs font-semibold text-zinc-600 dark:text-zinc-300" for="difference-{{ $transfer->id }}">{{ $t('نوع الفرق', 'Difference type') }}</label><select id="difference-{{ $transfer->id }}" name="difference_type" class="mt-1 w-full rounded-lg border-zinc-300 text-sm dark:border-zinc-700 dark:bg-zinc-900"><option value="">{{ $t('بدون فرق', 'No difference') }}</option><option value="shortage">{{ $t('عجز', 'Shortage') }}</option><option value="damage">{{ $t('تالف', 'Damage') }}</option><option value="refusal">{{ $t('رفض', 'Refusal') }}</option></select></div><div><label class="text-xs font-semibold text-zinc-600 dark:text-zinc-300" for="reason-{{ $transfer->id }}">{{ $t('سبب الفرق', 'Difference reason') }}</label><input id="reason-{{ $transfer->id }}" name="difference_reason" class="mt-1 w-full rounded-lg border-zinc-300 text-sm dark:border-zinc-700 dark:bg-zinc-900"></div><div class="flex items-end"><button class="w-full rounded-xl bg-emerald-700 px-3 py-2 text-xs font-bold text-white hover:bg-emerald-800" type="submit">{{ $t('تسجيل الاستلام', 'Record receipt') }}</button></div></form>@endif
                    </div>

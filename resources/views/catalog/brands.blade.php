...
<section class="catalog-screen w-full">
    <x-page-header :title="__('Brand Masters')" :description="__('Maintain bilingual brand identity with dependency-aware status changes.')">
        <x-slot:actions>@can('products_categories_brands.create')<flux:button icon="plus" variant="primary" wire:click="openCreateBrandModal">{{ __('Add brand') }}</flux:button>@endcan</x-slot:actions>
    </x-page-header>

...
### Server baseline data

For a clean server installation, run:
...
`DatabaseSeeder` is transactional and idempotent in every Laravel environment.
It creates the authorization catalog, `admin` / `admin@instaparty.online`, a
minimal Toy & Joy company, main branch, selling store, warehouse, cash drawer,
cash/card methods, a zero-tax default, standard document sequences, and a
browser-print profile. It does not create catalog, supplier, stock, customer,
sale, invoice, payment, booking, or other transactional data. Re-running it
never resets the existing administrator password or updates existing baseline
records.

The initial administrator password is `ToyJoy!Bootstrap2026`. Change it and
enable MFA immediately after the first sign-in. The application does not yet
enforce a first-login password-change screen.
...
in staging or Production. The default `DatabaseSeeder` does not call this
seeder.

...
final class RejectPartyWalletAdjustmentAction
{
    public function execute(User $approver, ApprovalRecord $approval, string $note): void
    {
        ...
        DB::transaction(function () use ($approver, $approval, $note): void {
            ...
            app(ApprovalRecordTransition::class)->execute(
                ...
                expectedSourceVersion: (string) $adjustment->lock_version, expectedSourceHash: (string) $approval->source_hash,
            ...
            app(RecordAuditEvent::class)->execute(
                category: 'customer_value', event: 'party_wallet_adjustment_rejected', source: $adjustment,
                ...
                reasonText: $note, metadata: ['wallet' => 'party', 'approval_record_id' => $approval->id, 'customer_id' => $adjustment->customer_id],
            );
        }, 5);
    }
}

...
final class SavePartyInvoiceAction
{
    ...
    public function execute(User $actor, PartyInvoice $invoice, array $data): PartyInvoice
    {
        ...
        return DB::transaction(function () use ($actor, $invoice, $data): PartyInvoice {
            ...
            if (! in_array($invoice->state, ['draft', 'active_working'], true) || $invoice->booking->isClosed()) throw new InvalidArgumentException(__('This Party invoice is locked after final close or operational freeze.'));
            $before = $invoice->only(['state', 'subtotal', 'total_amount', 'notes']);
            if (array_key_exists('notes', $data)) $invoice->update(['notes' => filled($data['notes']) ? trim((string) $data['notes']) : null, 'updated_by' => $actor->id]);
            if (array_key_exists('lines', $data)) $this->replaceLines($actor, $invoice, $data['lines']);
    ...
    {
        if (! is_array($raw) || $raw === []) throw new InvalidArgumentException(__('A Party invoice requires at least one line.'));
        ...
        foreach (array_values($raw) as $index => $line) {
            if (! is_array($line)) throw new InvalidArgumentException(__('Each Party line must be structured.'));
            ...
            if ($description === '' || ! preg_match('/^(?:0|[1-9]\d*)(?:\.\d{1,6})?$/', $quantity) || bccomp($quantity, '0', 6) <= 0 || ! preg_match('/^(?:0|[1-9]\d*)(?:\.\d{1,4})?$/', $price)) throw new InvalidArgumentException(__('Party line description, quantity, and price are invalid.'));
            ...
            if ($rentalAssetId !== null) {
                ...
                if ($asset === null) throw new InvalidArgumentException(__('The selected rental asset is not available in this Party scope.'));
                if ($existingLine?->asset_reservation_id !== null && (int) $existingLine->rental_asset_id !== $rentalAssetId) throw new InvalidArgumentException(__('Changing a reserved Party asset requires the approved reschedule flow.'));
                if ($invoice->state === 'active_working' && $reservationId === null) throw new InvalidArgumentException(__('A new Party rental asset must be reserved through booking confirmation before operation.'));
            }
            $attributes = ['line_type' => $type, 'product_id' => filled($line['product_id'] ?? null) ? (int) $line['product_id'] : null, 'rental_asset_id' => $rentalAssetId, 'asset_reservation_id' => $reservationId, 'description_ar' => trim((string) ($line['description_ar'] ?? $description)), 'description_en' => trim((string) ($line['description_en'] ?? $description)), 'quantity' => bcadd($quantity, '0', 6), 'unit_price' => bcadd($price, '0', 4), 'discount_amount' => '0.0000', 'tax_amount' => '0.0000', 'line_total' => bcmul($quantity, $price, 4), 'resource_key' => $asset?->code ?? (filled($line['resource_key'] ?? null) ? trim((string) $line['resource_key']) : null), 'source_reference' => filled($line['source_reference'] ?? null) ? trim((string) $line['source_reference']) : null];
            if (isset($existing[$number])) $existing[$number]->mutateDraft($attributes); else PartyInvoiceLine::query()->create(['party_invoice_id' => $invoice->id, 'line_number' => $number, ...$attributes]);
            $seen[$number] = true;
        }
        foreach ($existing as $number => $line) if (! isset($seen[$number])) $line->deleteDraft();
    }
}
